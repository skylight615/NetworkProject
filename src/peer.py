...
def process_inbound_udp(sock):
    ...
    if Type == 0:
        ...
        # send back GET pkt
        get_header = struct.pack("HBBHHII", socket.htons(Magic), Team, 2 , socket.htons(HEADER_LEN), socket.htons(HEADER_LEN+len(get_chunk_hash)), socket.htonl(0), socket.htonl(0))
