...
def process_inbound_udp(sock):
    ...
    if Type == 0:
        ...
        # send back IHAVE pkt
            ihave_header = struct.pack("HBBHHII", socket.htons(Magic), Team, 1, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN+len(whohas_chunk_hash)), socket.htonl(0), socket.htonl(0))
        ...
        # send back DATA
        data_header = struct.pack("HBBHHII", socket.htons(Magic), Team, 3, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN), socket.htonl(1), 0)
        ...
        if (ack_num)*MAX_PAYLOAD >= CHUNK_DATA_SIZE:
            ...
            # send next data
            data_header = struct.pack("HBBHHII", socket.htons(Magic), Team,  3, socket.htons(HEADER_LEN), socket.htons(HEADER_LEN+len(next_data)), socket.htonl(ack_num+1), 0)
